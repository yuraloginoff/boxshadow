...
angular
  .module("Boxshadow.controllers", [])

  .controller("BoxshadowController", [
    "$scope",
    "ColorConvert",
    function($scope, ColorConvert) {
      $scope.shadows = [];
        ...
        function(newValues, oldValues, scope) {
          ...
          for (var i = 0; i < newValues.length; i++) {
            ...
            if (newValues[i].colorModeSelected.type === "hex") {
              ...
              outputColor =
                "rgba(" +
                newValues[i].colorRGB.r +
                "," +
                newValues[i].colorRGB.g +
                "," +
                newValues[i].colorRGB.b +
                "," +
                newValues[i].opacity +
                ")";
            }
            ...
            propStr =
              ...
              (newValues[i].blur || 0) +
              "px " +
              (newValues[i].spread || 0) +
              "px " +
              outputColor +
              (newValues[i].inset ? " inset" : "");
            properties.push(propStr);
          }

          outputProperties = properties.join(", ");

          $scope.output = "box-shadow: " + outputProperties + ";\n";
        },
        true
      );
    }
